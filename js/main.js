//P.S. код получился большим, в реальном проекте наверное писал бы менее унифицировано и по-проще

const UrlData = "./data/data.json"; //для fetch запроса
...
const config = {
  ...
  arrData: [], //массив для сортировки
};

//идея заключается в том,что прислать могут почти любой json файл, все что нужно, это отредактировать обьект config
//к форме эту фишку прикрутить пока не удалось(

start(json()); // для запуска локально
//json определён внизу файла
...
function start(arr) {
  ...
  config.arrData = arr.map((elem) => elem); // создаем клон массива для сортировки
  //менять входын данные не лучшая практика
  //но спорное решение по созданию копии большого массива данных
  createTable(config.arrData);//отрисовка таблицы
  // запуск функции с прорисовкой таблицы
  createButtonChangePage(divForTable); //создание кнопок переключение страниц
  createButtonChangeColumns(divForTable); //создание кнопок для удаления\добавления столбцов таблицы
}
...
function createTable() {
  ...
  let columnsName = document.createElement("tr"); // создаем строку для заголовков
  ...
  for (let key of config.columnsName) {
    ...
    td.textContent = key; //присваиваем значение из config
    td.addEventListener("click", () => {
      //добавляем слушатель событий с запуском сортировки и пересозданием страницы
      sortedRows(config.columns[config.columnsName.indexOf(key)]);
      let table = document.querySelector("table");
      table.remove();
      config.pageNow = 0;
      changeActivePage(); //меняем выделение на кнопках страницы
      createTable();
    });
  ...
  createPage(arrChunk(config.arrData, 10)[config.pageNow], tbody); //создаем страницу таблицы
  //на вход кусочек от всего файла и локацию куда добавлять
}

function arrChunk(arr, chunk) {
  //функция по разделению одного массива на несколько
  //chunk - количество элементов в одном массиве
  let res = [];
  for (let i = 0; i < arr.length; i += chunk) {
    res.push(arr.slice(i, i + chunk));
  }
  return res; //возвращяет массив
}

function createPage(arr) {
  //создание строчек с данными в таблице
  let tb = document.querySelector("tbody");
  if (tb.querySelectorAll("tr").length) {
    //удаление старых данных для перерисовки
...
function createLine(location, obj) {
  //создание строки в локации из обьекта
  let tr = document.createElement("tr");
  tr.addEventListener("click", () => createForm(obj));
  getProp(obj); //функция создает ячейку если нету свойства в исключениях
  function getProp(o) {
    ...
    for (let prop in o) {
      if (typeof o[prop] === "object") {
        ...
        let td = document.createElement("td");
        if (prop === "eyeColor") {
          //заменя текста цвета на сам цвет в ячйке
          td.style.backgroundColor = o[prop];
        } else {
          td.textContent = o[prop];
        }
        td.setAttribute("class", prop); //присвоение класса свойства к каждой ячейке
        //для дальнейшего взаимодействия
        tr.append(td);
      }
    }
  }
  location.appendChild(tr); //добавление всей строки
}

function sortedRows(columnName) {
  //функция сортировки
  //принимает название колонки, сортирует сам обьект
  function getPath(object, key) {
    //функция для поиска пути к свойству внутри обьекта
    //запоминаем,чтобы каждый раз внутри сортировки не искать
    //при этом внутри обьекта не важна вложенность
    //и не важен сам обьект, нужно только название свойства
    let path = [];
    function iter(o, p) {
    ...
    iter(object, []);
    return path; //возвращяет путь в виде массива строк
  }

  let path = getPath(config.arrData[0], columnName); //узкое место, тут преполагаем, что обьекты в json файле одинаковы

  config.arrData.sort((objA, objB) => {
    //сама сортировка глобального массива
    let a = path.reduce(
      (sumObj, currentValue) => (sumObj = sumObj[currentValue]),
      objA //идем внутрь обьекта по пути
    );
    let b = path.reduce(
      (sumObj, currentValue) => (sumObj = sumObj[currentValue]),
      objB
    );
    return a.localeCompare(b); //сравнение
  });
}

function createForm(person) {
  //функция по созданию формы по клику
  //не совсем понял, что требовалось от формы

  if (!document.querySelectorAll("form").length) {
    body.insertAdjacentHTML(
      "beforeend",
      `<div class="change-content">
      <form name="${person.id}" onsubmit="changeData(this);return false" >
        <label for="firstName">Изменить имя ${person.name.firstName}:</label><br>
        <input type="text" id="firstName" name="firstName" ><br>
        <label for="lastName">Изменить фамилию ${person.name.lastName}:</label><br>
    ...
    let btnCloseForm = document.createElement("button"); //создаем кнопку для закрытия формы
    btnCloseForm.textContent = "Закрыть форму";
    let form = document.querySelector(".change-content");
    btnCloseForm.addEventListener("click", () => {
      form.remove();
    });
    form.appendChild(btnCloseForm);
  } else {
    let form = document.querySelector(".change-content");
    form.remove();
    createForm(person);
...
function changeData(formHtml) {
  console.log(formHtml.name);
  const form = new FormData(formHtml);
  let objIndex = config.arrData.findIndex((item) => item.id === formHtml.name); //находим индекс обьекта для взаимодействия с ним
  for (let pair of form.entries()) {
    //проходимся по всей форме
    if (pair[1] === "") {
      //пропуск пустых строк
      continue;
    }
    function getPath(object, key) {
      // и опять функция для поиска пути к свойству внутри обьекта
      //наверное, это можно реализовать проще
      let path = [];
      function iter(o, p) {
        //рекурсивная функция для поиска пути
        if (typeof o === "object") {
    ...
    let path = getPath(config.arrData[objIndex], pair[0]); //узкое место, тут преполагаем, что обьекты в json файле одинаковы

    let property = config.arrData[objIndex];

    for (let i = 0; i < path.length; i++) {
      //проходимся по пути к свойству
      if (path.length - i === 1) {
        property[path[i]] = pair[1]; //переписываем значение
      }
      property = property[path[i]];
    }
  }
  let tbody = document.querySelector("tbody");
  createPage(arrChunk(config.arrData, 10)[config.pageNow], tbody); //перерисовываем страницу
}

function createButtonChangePage(location) {
  //создает кнопки по переключению страницы и их логику
  //на вход подается локация для создания кнопок
  let tbody = document.querySelector("tbody");
  ...
  for (let i = 0; i < arrChunk(config.arrData, 10).length; i++) {
    //создание кнопок по количеству разбитых массивов
    let btn = document.createElement("button");
    btn.setAttribute("class", `btn${i + 1}`);
    btn.textContent = i + 1;
    btn.addEventListener("click", () => {
      //слушатель для переключения страниц
      config.pageNow = i;
      createPage(arrChunk(config.arrData, 10)[i], tbody);
      changeActivePage(); //меняет выделение на самих кнопках
    });
    ChangePage.appendChild(btn);
  }
  let btnNow = document.querySelector(`.btn${config.pageNow + 1}`);
  btnNow.classList.add("btnActive"); //в начале прорисовки добавлет выделение на стартовую страницу таблицы
}

function changeActivePage() {
  //изменение выделения кнопки с выбором страницы
  let btnLast = document.querySelector(".btnActive");
  btnLast.classList.remove("btnActive");
  let btnNow = document.querySelector(`.btn${config.pageNow + 1}`);
  btnNow.classList.add("btnActive");
}

function createButtonChangeColumns(location) {
  //функция для показа скрытия колонок(и создает сами кнопки)
  //на вход локация для создания кнопок
  ...
  for (let key of config.columnsName) {
    //создание кнопок по конфигу
    let btn = document.createElement("button");
    btn.textContent = key;
    btn.addEventListener("click", () => {
      if (
        //проверка удалять или добавлять столбец
        config.exclusion.indexOf(
          config.columns[config.columnsName.indexOf(key)]
        ) === -1 //такая сложная проверка так, как названия колонок в цикле на русском, а в конфиге в исключениях на английском
      ) {
      ...
      let table = document.querySelector("table");
      table.remove(); //пересодание таблицы уже с новыми исключениями в конфиге
      createTable();
    });
